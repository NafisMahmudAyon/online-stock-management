    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-300">
          <thead>
            <tr>
              <th className="py-2 px-4 border-b cursor-pointer" onClick={() => handleSort('entryid')}>Entry ID</th>
              <th className="py-2 px-4 border-b cursor-pointer" onClick={() => handleSort('productid')}>Product ID</th>
              <th className="py-2 px-4 border-b cursor-pointer" onClick={() => handleSort('supplierid')}>Supplier ID</th>
              <th className="py-2 px-4 border-b cursor-pointer" onClick={() => handleSort('producttypeid')}>Product Type ID</th>
              <th className="py-2 px-4 border-b cursor-pointer" onClick={() => handleSort('purchasedate')}>Purchase Date</th>
              <th className="py-2 px-4 border-b cursor-pointer" onClick={() => handleSort('purchaseunitprice')}>Purchase Unit Price</th>
              <th className="py-2 px-4 border-b cursor-pointer" onClick={() => handleSort('purchasequantity')}>Purchase Quantity</th>
          ...
          <tbody>
            {sortedEntries.map((entry) => (
              <tr key={entry.entryid}>
                ...
                <td className="py-2 px-4 border-b">{entry.purchaseunitprice}</td>
                        ...
                        <li key={index}>
                          {variationGroup.map((variation) => (
                            <div key={variation.supplyvariationid}>
                              ...
                              <p>Attributes: {parseAttributes(variation.attributes)}</p>
